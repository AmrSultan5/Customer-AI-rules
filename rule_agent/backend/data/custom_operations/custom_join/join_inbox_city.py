...
from governance_data_quality_processes.custom_operation_configs.custom_join.join_inbox_city_config import JoinInboxCityConfig, JoinInboxCityOperationConfig
from datamesh_transformation.common.context import TransformationContext


class JoinInboxCityOperation(BaseOperation):
    """
    Join multiple dataframes based on inbox and matched city condition
    """

    @log_execution_time
    def transform(self, ctx: TransformationContext) -> Optional[DataFrame]:
        assert isinstance(self._config, JoinInboxCityOperationConfig)
    ...
    def _get_join_condition(self, ctx: TransformationContext, df: DataFrame, cfg: JoinInboxCityConfig) -> List[Column]:
        """
    ...
    def _get_drop_condition(self, cfg: JoinInboxCityConfig) -> List[str]:
        return [
            col_left
